...
int main(int argc, char **argv)
{
    ...
    unsigned int flags;
    size_t bytes;
    float *a, *b, *c, *d;                      // Pinned memory allocated on the CPU
    ...
    else
    {
        ...
        checkCudaErrors(cudaHostAlloc((void **)&d, bytes, 0));
#endif
    }
    ...
    printf("Time for vectorAddGPU execute (ms): %f\n", ms);
    //checkCudaErrors(cudaDeviceSynchronize());
    getLastCudaError("vectorAddGPU() execution failed");



    // baseline case -  execute  mapped
    checkCuda( cudaEventRecord(startEvent,0) );
    ...
    // baseline case -  execute  copy
    float *d_x, *d_y, *d_z;
    checkCudaErrors(cudaMalloc((void **)&d_x, bytes));
    checkCudaErrors(cudaMalloc((void **)&d_y, bytes));
    checkCudaErrors(cudaMalloc((void **)&d_z, bytes));

    checkCuda( cudaEventRecord(startEvent,0) );
    checkCuda( cudaMemcpyAsync(&d_x, &a_UA,
                          bytes, cudaMemcpyHostToDevice,
                          stream) );
   checkCuda( cudaMemcpyAsync(&d_y, &b_UA,
                          bytes, cudaMemcpyHostToDevice,
                          stream) );
    vectorAddGPU<<<grid, block, 0, stream>>>(d_x, d_y, d_z, nelem);
    checkCuda( cudaMemcpyAsync(&c_UA, &d_z,
                          bytes, cudaMemcpyDeviceToHost,
                          stream) );
    checkCuda( cudaEventRecord(stopEvent, 0) );
    checkCuda( cudaEventSynchronize(stopEvent) );
    checkCuda( cudaEventElapsedTime(&ms, startEvent, stopEvent) );
    printf("Time for vectorAddGPU async with pageable memcpy execute (ms): %f\n", ms);
