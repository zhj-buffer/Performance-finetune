...
enum memcpyKind { DEVICE_TO_HOST, HOST_TO_DEVICE, DEVICE_TO_DEVICE, HOST_TO_HOST };
...
float testHostToHostTransfer(unsigned int memSize);
...
int runTest(const int argc, const char **argv)
{
    ...
    if (checkCmdLineFlag(argc, argv, "htoh"))
    {
        htoh = true;
    }
    ...
    {
        testBandwidth((unsigned int)start, (unsigned int)end, (unsigned int)increment,
                      mode, HOST_TO_HOST, printmode, memMode, startDevice, endDevice, wc);
    }
    ...
    for (int currentDevice = startDevice; currentDevice <= endDevice; currentDevice++)
    {
        ...
        for (unsigned int i = 0; i < count; i++)
        {
            ...
            switch (kind)
            {
                ...
                case HOST_TO_HOST:
                    bandwidths[i] += testHostToHostTransfer(memSizes[i]);
                    break;
            }
        }
    ...
    for (int currentDevice = startDevice; currentDevice <= endDevice; currentDevice++)
    {
        ...
        while (memSize <= SHMOO_MEMSIZE_MAX)
        {
            ...
            switch (kind)
            {
                ...
                case HOST_TO_HOST:
                    bandwidths[iteration] += testHostToHostTransfer(memSizes[iteration]);
                    break;
            }
...
///////////////////////////////////////////////////////////////////////////////
//! test the bandwidth of a Host to Host memcopy of a specific size
///////////////////////////////////////////////////////////////////////////////
float
testHostToHostTransfer(unsigned int memSize)
{
    StopWatchInterface *timer = NULL;
    float elapsedTimeInMs = 0.0f;
    float bandwidthInGBs = 0.0f;
    ...
    //allocate host memory
    unsigned char *h_idata = (unsigned char *)malloc(memSize);
    unsigned char *h_idata_dst = (unsigned char *)malloc(memSize);

    if (h_idata == 0 || h_idata_dst == 0)
    {
        fprintf(stderr, "Not enough memory avaialable on host to run test!\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        h_idata[i] = (unsigned char)(i & 0xff);
        h_idata_dst[i] = (unsigned char)(i & 0x00);
    }
    ...
    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
    {
        if (!memcpy(h_idata_dst, h_idata, memSize)) 
		exit(EXIT_FAILURE);
    }
...
void printHelp(void)
{
    ...
    printf("--htoh\tMeasure host to host transfers\n");
#if CUDART_VERSION >= 2020
